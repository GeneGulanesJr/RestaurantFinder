...
- **GET `/api/execute` contract**: Single endpoint for UI and external consumers; success JSON with `results` and `interpreted`; error responses 401, 400, 422, 502.
...
- `api-contract`: GET `/api/execute?message=...&code=pioneerdevai`; success and error response shapes; same endpoint for UI and JSON consumers.
